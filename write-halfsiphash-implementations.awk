...
function write_sats_include_file(rounds_pairs,  i, f)
{
  ...
  f = "siphash/SATS/include/halfsiphash-implementations.inc"
...
{
  write_sats_include_m_n_outlen(f, 4, crounds, drounds)
  print > f
...
{
  write_sats_include_c_d_outlen(f, 4)
  print > f
...
{
  print "fun" > f
  print "halfsiphash_" crounds "_" drounds "_" (8 * outlen) > f
  print "        {inlen : int}" > f
  print "        (input : &RD(@[byte][inlen])," > f
  print "         inlen : size_t inlen," > f
  print "         key   : &RD(@[byte][8])) :<!ref> "       \
    ((outlen == 4) ? "uint32" : "@(uint32, uint32)") > f
  if (outlen == 4)
    {
      print > f
      print "overload halfsiphash_" crounds "_" drounds                 \
        " with halfsiphash_" crounds "_" drounds "_" (8 * outlen) > f
    }
}
...
{
  print "fun" > f
  print "halfsiphash_" crounds "_" drounds "_output" > f
  print "        {inlen  : int}" > f
  print "        {outlen : int | outlen == 4 || outlen == 8}" > f
  print "        (input  : &RD(@[byte][inlen])," > f
  print "         inlen  : size_t inlen," > f
  print "         key    : &RD(@[byte][8])," > f
  print "         output : &(@[byte?][outlen]) >> @[byte][outlen]," > f
  print "         outlen : size_t outlen) :<!refwrt> void" > f
  print > f
  print "overload halfsiphash_" crounds "_" drounds         \
    " with halfsiphash_" crounds "_" drounds "_output" > f
}

function write_sats_include_c_d_outlen(f, outlen)
{
  print "fun" > f
  print "halfsiphash_c_d_" (8 * outlen) > f
  print "        {inlen   : int}" > f
  print "        (input   : &RD(@[byte][inlen])," > f
  print "         inlen   : size_t inlen," > f
  print "         key     : &RD(@[byte][8])," > f
  ...
  if (outlen == 4)
    {
      print > f
      print "overload halfsiphash_c_d with halfsiphash_c_d_" (8 * outlen) > f
    }
}
...
{
  print "fun" > f
  print "halfsiphash_c_d_output" > f
  print "        {inlen   : int}" > f
  print "        {outlen  : int | outlen == 4 || outlen == 8}" > f
  print "        (input   : &RD(@[byte][inlen])," > f
  print "         inlen   : size_t inlen," > f
  ...
  print "         outlen  : size_t outlen) :<!refwrt> void" > f
  print > f
  print "overload halfsiphash_c_d with halfsiphash_c_d_output" > f
}
...
{
  write_dats_m_n_outlen(4, crounds, drounds)
...
function write_dats_m_n_outlen(outlen, crounds, drounds,  f)
{
  ...
  f = "siphash/DATS/halfsiphash_" crounds "_" drounds "_" (8 * outlen) ".dats"
  write_dats_header(f)
  print > f
  print "local" > f
  print "  implement halfsiphash$crounds<> () = " crounds "U" > f
  print "  implement halfsiphash$drounds<> () = " drounds "U" > f
  print "in" > f
  print "  implement" > f
  print "  halfsiphash_" crounds "_" drounds "_" (8 * outlen) " (input, inlen, key) =" > f
  print "    halfsiphash_" (8 * outlen) "<> (input, inlen, key)" > f
  print "end" > f
}
...
{
  system("mkdir -p siphash/DATS")
  f = "siphash/DATS/halfsiphash_c_d_" (8 * outlen) ".dats"
  write_dats_header(f)
  print > f
  print "implement" > f
  print "halfsiphash_c_d_" (8 * outlen) " (input, inlen, key, crounds, drounds) =" > f
  print "  let" > f
  print "    implement halfsiphash$crounds<> () = crounds" > f
  print "    implement halfsiphash$drounds<> () = drounds" > f
  print "  in" > f
  print "    halfsiphash_" (8 * outlen) "<> (input, inlen, key)" > f
  print "  end" > f
}
...
{
  system("mkdir -p siphash/DATS")
  f = "siphash/DATS/halfsiphash_" crounds "_" drounds "_output.dats"
  write_dats_header(f)
  print > f
  print "local" > f
  print "  implement halfsiphash$crounds<> () = " crounds "U" > f
  ...
  print "in" > f
  print "  implement" > f
  print "  halfsiphash_" crounds "_" drounds "_output (input, inlen, key, output, outlen) =" > f
  print "    halfsiphash<> (input, inlen, key, output, outlen)" > f
  print "end" > f
}
...
{
  system("mkdir -p siphash/DATS")
  f = "siphash/DATS/halfsiphash_c_d_output.dats"
  write_dats_header(f)
  print > f
  print "implement" > f
  print "halfsiphash_c_d_output (input, inlen, key, crounds, drounds," > f
...
function write_dats_header(f)
{
  ...
  print "staload \"siphash/SATS/halfsiphash.sats\"" > f
  print "staload _ = \"siphash/DATS/halfsiphash.dats\"" > f
}
